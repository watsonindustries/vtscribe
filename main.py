...
async def accept_job(request: Request, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
    ...
    job_spec = config.JobSpec(
        source=source,
        whisper_model=config.SUPPORTED_WHISPER_MODELS[model_name])

    call = transcribe.spawn(job_spec)
...
async def transcribe(job_spec: config.JobSpec):
    metadata = transcribe_vod.call(job_spec)
...
def transcribe_vod(job_spec: config.JobSpec):
    ...
    try:
        ...
        result_metadata: dict = do_transcribe.call(
            input_raw_file_path,
            model=job_spec.whisper_model,
            result_path=result_path)

        upload_to_obj_storage.call(
            os.path.join('transcripts', result_metadata['result_file_name']),
            result_metadata['result_path']
        )
        upload_to_obj_storage.call(
            os.path.join('transcripts', result_metadata['result_vtt_name']),
            result_metadata['result_vtt_path']
        )
...
def do_transcribe(input_audio_file_path: str, model: config.ModelSpec, result_path: str = "result.json") -> dict:
    ...
    segment_gen = split_silences(output_file)
    ...
    result = {
        "text": output_text,
        "segments": util.coalesce_short_transcript_segments(output_segments),
        "language": "en",
    }

    end_time = time.time()

    logger.info(f"Writing openai/whisper JSON transcription to {result_path}")
    ...
    bucket_cdn_url = util.bucket_cdn_url(config.DO_DEFAULT_BUCKET_NAME)
    result_json_name = os.path.basename(result_path)
    result_vtt_name = os.path.basename(result_vtt_path)

    metadata = {
        'result_path': result_path,
        'result_vtt_path': result_vtt_path,
        'result_file_name': result_json_name,
        'result_vtt_name': result_vtt_name,
        'result_json_url': os.path.join(bucket_cdn_url, 'transcripts', result_json_name),
        'result_vtt_url': os.path.join(bucket_cdn_url, 'transcripts', result_vtt_name),
        'job_info': {
